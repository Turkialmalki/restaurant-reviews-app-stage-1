  ...
  fetchRestaurantFromURL((error, restaurant) => {
    if (error) { // Got an error!
      ...
      L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.jpg70?access_token={mapboxToken}', {
        mapboxToken: 'pk.eyJ1Ijoic2pva29sYWRldngiLCJhIjoiY2psMmN6ZXl0MXBidDN4cXAyNHJmazQ0bSJ9.6BGkA_J9PN0vLsxzOtAk8g',
        maxZoom: 18,
...
fillRestaurantHTML = (restaurant = self.restaurant) => {
  ...
  // Add in image alt tag for accessibility
...
fillReviewsHTML = (reviews = self.restaurant.reviews) => {
  ...
  /* Comparative priority lowered (h3) */
...
createReviewHTML = (review) => {
  ...
  /* Add review name class */
  name.classList.add('review-name');
  li.appendChild(name);

  const date = document.createElement('p');
  date.innerHTML = review.date;
  /* Add review date class */
  date.classList.add('review-date');
  li.appendChild(date);

  const rating = document.createElement('p');
  rating.innerHTML = `Rating: ${review.rating}`;
  /* Add review rating class */
  rating.classList.add('review-name');
